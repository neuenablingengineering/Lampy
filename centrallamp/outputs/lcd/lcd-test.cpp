 ...
 *
 * A brief demonstration of the Raspberry Pi I2C interface, using the
 * SparkFun
 * Pi Wedge breakout board.
 *
 * Resources:
 *
 * This example makes use of the Wiring Pi library, which streamlines
 * the interface
 * to the the I/O pins on the Raspberry Pi, providing an API that is
 * similar to the
 ...
 *
 * The init call returns a standard file descriptor.  More detailed
 * configuration
 * of the interface can be performed using ioctl calls on that
 * descriptor.
 * See the wiringPi SPI implementation (wiringPi/wiringPiSPI.c) for
 * some examples.
 ...
 *
 * Then to run it, first the spi kernel module needs to be loaded.
 * This can be 
 ...
 * It shifts a 
 * bit through the display characters, lighting a single character of
 * each at a 
 * time.
 *
...
// channel is the wiringPi name for the chip select (or chip enable)
// pin.
